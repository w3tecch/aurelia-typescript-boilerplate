...
describe('aurelia skeleton app', function(): void {
  ...
  it('should load the page and display the initial page title', async () => {
    await expect(poSkeleton.getCurrentPageTitle()).toBe('Welcome | Translation Title');
  });
  ...
  it('should navigate to users page', async () => {
    ...
    await expect(poSkeleton.getCurrentPageTitle()).toBe('Github Users | Translation Title');
  });
});
